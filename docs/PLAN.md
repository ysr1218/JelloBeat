...
### Phase 4-5 — 드리블 (기획 확정, 미구현)

**목표:** 날아가는 젤리박스를 마우스 커서로 쳐서 농구공 드리블처럼 갖고 놀기.

- 커서가 움직이는 박스 위를 지나갈 때 커서 이동 방향·속도에 비례한 impulse를 velocity에 추가.
  클릭 없이 접촉만으로 반응 (자동 impulse 방식 채택).
- 박스 속도 임계값으로 두 모드 전환:
  - **드리블 모드** (빠름): 커서 접촉 시 impulse 적용, 드래그 비활성.
  - **잡기 모드** (느림/정지): 기존 드래그 동작 유지.
- 기존 velocity RAF 루프 + 마우스 이벤트 조합으로 구현 — 새 물리 시스템 불필요.
- 드리블 추가 후 스프링/스쿼시 수치 미세 조정 필요할 수 있음.

**설계 시 결정할 점:**
1. 드리블/잡기 모드 전환 속도 임계값 (STOP_THRESH와 별도 상수 or 공유)
2. 클릭스루 hit_rect가 빠르게 움직이는 박스를 따라가야 하므로 드리블 중 hit_rect 갱신 주기 단축 고려
3. 박스가 커서를 통과하는 순간만 impulse를 주는지, 커서가 박스 위에 있는 동안 계속 주는지
...
| 곡 스킵 시 YTM 세션이 순간 Other 상태 → 다른 세션으로 잠깐 전환됐다가 복귀, 곡 정보 찰나 깜빡임 | SMTC가 스킵 중 세션 상태를 일시적으로 Other로 보고 | Phase 7 모드 시스템에서 세션 전환 디바운스(유예 시간) 추가로 개선 가능 |
| 화면이 꺼졌다 켜지거나 모니터 구성이 바뀌면(절전 아님, 디스플레이 off→on) 젤리박스가 사라짐. 앱은 살아있고 작업표시줄엔 표시됨 | 디스플레이 재초기화 시 가상 데스크탑 창 크기/위치 계산이 틀어져 박스가 화면 밖 좌표로 이동하는 것으로 추정 | Phase 7에서 디스플레이 변경 이벤트 구독 시 `fit_to_virtual_desktop` 재호출 + 박스 위치를 화면 안으로 보정하는 로직으로 해결 예정 |
| 곡 전환 후 제목·앨범아트 표시까지 1~2초 지연 | SMTC가 음악 앱(YTM, Spotify)으로부터 새 곡 메타데이터를 받는 타이밍이 앱마다 다르며 구조적으로 제어 불가. 코드 내 디바운스 없음, 썸네일 직렬화 오버헤드는 미미 (50ms 미만) | 코드 쪽 최적화 여지 없음 — 음악 앱 쪽 SMTC 갱신 타이밍에 달림 |
